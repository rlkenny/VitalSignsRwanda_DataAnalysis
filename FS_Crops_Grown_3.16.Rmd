...
ase_when(ag4a_01 == TRUE ~ kg_peracre,
                                                              ag4a_01 == FALSE ~ ag4a_15/ag4a_08*ag4a_02))

```{r}
testing <-merge(joined_df, out_rwanda, all.x=T, all.y=F)



testing <- select(testing, hfias, hh_refno, ag4a_04, field_no, landscape_no, crop_name)

testing <- mutate(testing, ic = case_when(ag4a_04 == TRUE ~ 1, FALSE ~ 0))
```

```{r}
testing$ic[is.na(testing$ic)] <- 0

testing$ic <- as.numeric(testing$ic)
...
```{r}
testing_ag  <- aggregate(ic ~ hh_refno + hfias + landscape_no, FUN =  sum, data = testing)
...
```{r}
testing_ag2 <- aggregate(hfias ~ hh_refno + ic +landscape_no, FUN = mean, data= testing_ag)

testing_ag3 <- mutate(testing_ag2, sometimes_ic = case_when(ic > 0 ~ TRUE,
                                                              ic == 0 ~ FALSE))
...
gulp <- lm(testing_ag3$hfias ~ testing_ag3$sometimes_ic + testing_ag3$landscape_no)
summary(gulp)
...
inter <- filter(testing_ag3, sometimes_ic == TRUE)
dont <- filter(testing_ag3, sometimes_ic == FALSE)
...
fs_df1 <- mutate(testing_ag3, Level = ifelse(testing_ag3$hfias == 0 | testing_ag3$hfias == 1, 4, ifelse(testing_ag3$hfias > 1 & testing_ag3$hfias <= 18, 3, ifelse(testing_ag3$hfias > 18 & testing_ag3$hfias <= 36, 2, 1)))) 


fs_df2 <- mutate(testing, Level = ifelse(testing$hfias == 0 | testing$hfias == 1, 4, ifelse(testing$hfias > 1 & testing$hfias <= 18, 3, ifelse(testing$hfias > 18 & testing$hfias <= 36, 2, 1)))) %>% 
  na.omit(fs_df)

# View(fs_df)

#colourCount = length(unique(fs_df$crop_name))
...
FS_landscape <- ggplot(fs_df2, aes(x=landscape_no, fill=rev(ordered(Level))))+
...
FS_level_crops <- ggplot(fs_df2, aes(x=Level, fill=ordered(crop_name)))+
  geom_bar(stat="count", position = "fill")+
...
fs_inter_bar <- ggplot(fs_df1, aes(sometimes_ic, fill=ordered(Level)))+
  geom_bar(stat="count", position = "fill")+
